...
秒级删除时间不足以确定快速连续删除的先后顺序。如果两个不同 world ID 在同一秒进入回收站，按 Trash ID 字符串排序可能把更早条目误当作“最近删除”，导致 Undo 恢复错误世界。

因此 Manifest 必须保存跨会话 Unix 微秒时间戳；只有微秒时间也相同时，才使用自然 Trash ID 作为确定性 tie-break。

损坏 Manifest 对应的物理目录仍必须占用容量，否则损坏条目会绕过 32 条硬上限并造成无界磁盘增长。
...
- 回收站最多 32 个物理目录；
- 损坏 Manifest 仍占用容量并进入诊断；
- 满时返回 `trash_full`，不自动清理；
- Undo 按 Unix 微秒时间选择最近有效条目；
...
本轮将存档删除从“单击永久销毁”升级为“二次确认、完整目录原子回收、32 条容量、微秒级最近删除顺序、冲突安全恢复和可撤销 UI”。同时保留维护级永久删除兼容 API，避免把玩家安全功能扩散到测试和基础存档合同中。
