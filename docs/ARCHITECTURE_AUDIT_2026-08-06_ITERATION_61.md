...
**Resolution:** candidate identity uses commit, version, lengths and content hashes only. Paths are limited to stable file names and repository contract paths.
...
**Resolution:** create a canonical directory layout and validate the physical evidence files against the package artifact manifest.

### A-61-04 · Copy destinations must fail closed
...
### A-61-05 · Expected-file checks alone do not reject injection

A validator that only verifies required files can still accept an unexpected executable, script or misleading second package.

**Resolution:** recursively enumerate the directory and require an exact eleven-file payload plus `bundle-manifest.json`.

### A-61-06 · Portable paths require traversal and link defenses

A manifest path can be syntactically relative while escaping the root through `..`, a drive prefix or a reparse point.

**Resolution:** reject absolute/drive/parent paths, verify resolved paths remain beneath the bundle root and reject all reparse points.

### A-61-07 · The main roadmap lagged the merged implementation
...
- evidence files that do not match `artifact_manifest`;
...
- assemble and validate the complete portable directory;
- deliberately mutate evidence content;
- deliberately mutate candidate identity while updating the outer file hash;
- inject an unexpected file;
...
The repository-owned chain-of-custody gap is closed when the permanent workflow is green. External commercial qualification remains **HOLD** because transport integrity cannot substitute for independent people, physical machines or real interruption events.
