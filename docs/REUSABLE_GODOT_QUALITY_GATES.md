...
这些步骤此前在每个 YAML 中重复维护。一次 Godot 版本、等待包装器、日志判断或 Artifact 规则调整，需要修改多个文件，并容易出现某个专项继续使用旧命令的漂移。
...
后续专项可以复用同一模板，但首批六个调用仍作为“重复实现已经被收敛”的固定合同样本。
...
进程退出码为 0 不是充分成功条件。`run_godot_headless_test.ps1` 与 `run_godot_desktop_test.ps1` 都必须在报告 PASS 前扫描 stdout 和 stderr，以下内容任一出现即失败：
...
这条规则用于阻止脚本错误、解析错误和资源泄漏被 Godot 的零退出码掩盖。原始日志仍会先写入 Artifact 路径，失败证据不会丢失。
...
真实桌面脚本仍然负责产品输入、场景、保存、重载和 1024×576 截图断言。模板只负责可靠启动、等待、超时、致命诊断扫描和证据上传，不替代领域验收。
...
| 静态验证器失败 | 不安装 Godot、不运行后续领域或桌面 Job |
| 严格导入失败 | 不运行领域脚本 |
| 领域脚本超时/非零退出 | Job 失败并保留已声明日志 |
| 捕获日志包含脚本错误或资源泄漏 | 即使退出码为 0，Job 仍失败 |
...
调用方不使用 `secrets: inherit`，不会获得仓库写权限，也不会创建 Release、Tag 或提交。
...
- 所有输入和等待包装器存在；
- Headless 与 Desktop 包装器同时扫描 stdout/stderr 致命诊断；
- Checkout、Godot Setup 和 Artifact Upload 只在模板的两个 Job 中实现；
- 首批六个调用工作流不重新出现 `runs-on` 或 Setup 步骤；
