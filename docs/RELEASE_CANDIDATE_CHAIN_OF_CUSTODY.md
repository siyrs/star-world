...
Iteration 61 closes the transport and handoff gap between the Iteration 60 qualification collectors. Iteration 60 already proves that every submitted record belongs to one commit, EXE and PCK when the package is assembled. Iteration 61 adds one immutable candidate manifest and one directly inspectable portable bundle so the same final candidate can be copied between the reviewer, minimum machine, recommended machine, soak machine and fault laboratory without relying on file names or operator memory.
...
- copy the candidate and evidence without losing the original hashes;
- detect missing, extra, replaced, truncated or path-traversal files;
...
Absolute machine paths are not part of `candidate_id`. The same bytes therefore retain one identity after being copied to another machine.
...
Every collector already records the final package hashes. The Iteration 60 package assembler rejects mixed builds before creating `qualification-package.json`.
...
The assembler rejects a non-empty output directory to prevent stale evidence from silently surviving a new bundle. Every copied file is hashed again after the copy.
...
- the exact required file set, with no extra files;
- no symbolic links or reparse points;
- safe relative paths with no parent-directory traversal;
...
- actual evidence-file hashes against `artifact_manifest`;
...
- portable bundle assembly and revalidation;
- evidence-file tamper rejection;
