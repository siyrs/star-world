...
v1/v2 旧记录在首次迁移时会按去重后的历史顺序分配 sequence。v3 及更高版本会保留合法、递增的 sequence 及其空洞，确保玩家看到的发现编号不会因为保存、载入或日志渲染而变化；只有缺失、重复或逆序的损坏序号才会被修复。
...
- 重复 key 最新项胜出；
- sequence 与世界时间；
- v3 合法 sequence 空洞跨保存、载入和日志渲染保持不变；
