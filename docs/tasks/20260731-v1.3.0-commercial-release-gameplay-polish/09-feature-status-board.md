...
|---|---|---|---|---|---|---|
| FP-001 | complete | passed | accepted-repository | build/import/log/save/automation discovery and permanent gates | 2026-08-07 | current source, Godot 4.7 and Windows release paths are governed |
| FP-002 | complete | passed | accepted-repository | five profiles, content registries and coverage matrices | 2026-08-07 | formal content inventory is deterministic and validated |
| FP-003 | complete | passed | accepted-repository | fresh export, launch, quit, create-world and Release lifecycle report | 2026-08-07 | scene/world/save/quit timing and resource evidence are permanent |
| FP-004 | complete | passed | accepted-repository | five-profile production routes and closed-loop player journeys | 2026-08-07 | no post-spawn transport or direct transform writes in final route evidence |
| FP-005 | complete | passed | accepted-repository | collision, seam, fall, block shape and recovery regressions | 2026-08-07 | includes exact sky-island descent and connected partial shapes |
...
## Iteration 62 closure

- A deterministic Promotion Pin binds the release-owner selection to candidate ID, chain bundle ID, package ID, commit, version and final binary hashes.
- `-RequireReleaseGate` now requires an externally retained expected pin rather than accepting internal consistency alone.
- The accepted Iteration 61 chain remains unchanged inside an outer Promotion Bundle.
- Frozen snapshots of release qualification policy, `project.godot` and `export_presets.cfg` allow the nested candidate chain to be validated without trusting the receiver's current checkout.
- Outer promotion validation rejects missing/extra files, hidden injections, unsafe paths, reparse points and hash/length drift and derives one deterministic `promotion_id`.
