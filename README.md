...
- **最新版本：v1.3.0**（2026-08-02）— [GitHub Release](https://github.com/siyrs/star-world/releases/tag/v1.3.0)
- 下载 `StarWorld-Windows-x86_64.zip`，解压运行 `StarWorld.exe`；可用附带的 `.sha256` 校验文件。
- 变更记录见 [CHANGELOG](docs/CHANGELOG.md)；发布验收证据见 `qa/final-release-report.md`（47/47 OpenSpec 门槛、全量回归 113/113、fresh export smoke 16/16）。
...
- **熔岩是危险地形**：接触持续灼烧（每 0.5s 4 点伤害），离开即停，长时间浸泡致死并可重生；水仍为安全流体。
- 出生点确定性评分（净空/邻近可走/视野/危险方块回避），五地图全部安全出生。
- 全部交互按钮（normal/hover/pressed/focus）有效对比度 ≥ 4.5:1，支持键盘导航、控制器焦点与高 DPI 缩放。
...
当前版本定位为可持续扩展的单人核心。v1.3.0 已具备：熔岩灼烧（危险地形）、出生质量确定性评分、全部交互按钮对比度 ≥ 4.5:1、统一运行与保存健康报告、跨 Chunk 结构完整性、有界世界目录/存档恢复与 30 分钟长稳证据。

仍为边界：水仍为安全流体（无氧气/溺水）；远程、多段与 Boss 攻击尚未引入；装备、休息和修理仍有部分生命周期留在 ServiceHub 继承层。Machine Base 已支持 Furnace、Stonecutter、原子 Capability、活跃索引和有界相邻箱子自动化，但尚未引入电网、管线、跨 Chunk 物流或自动路径搜索。

下一阶段重点：水面下的深水/潜水玩法决策、装备/休息生命周期的下沉、更大存档规模的长稳采样（120 分钟级），以及在真实使用率与预算证据出现后再决定是否扩展自动化。多人和大型商业内容库不在当前阶段范围内。
