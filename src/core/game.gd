...
func _ready() -> void:
	...
	if service_hub.has_signal("return_to_menu_requested"):
		service_hub.connect("return_to_menu_requested", Callable(self, "_on_return_to_menu_requested"))
...
func start_world(profile_id: String, seed: int, world_id: String, saved_state: Dictionary = {}) -> bool:
	_ensure_core_nodes()
	...
	world.call("start_world", current_profile_id, current_seed, current_world_id, current_saved_state)
	player.call("bind_world", world)
	var spawn_position: Vector3 = world.call("get_spawn_position")
	var player_state: Dictionary = current_saved_state.get("player", {})
	if player_state.has("position"):
		spawn_position = _array_to_vector3(player_state.get("position", []), spawn_position)
...
func _attach_gameplay_services() -> void:
	var inventory = service_hub.get("inventory")
	var survival = service_hub.get("survival")
	if inventory is Node:
		player.call("bind_inventory", inventory)
	if survival is Node:
		player.call("bind_survival", survival)
	if service_hub.has_method("attach_game"):
		service_hub.call("attach_game", world, player, sun, world_environment, Callable(world, "resolve_ground_position"))
