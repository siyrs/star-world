...
var policy: RefCounted = PolicyScript.new()
...
func setup(p_item_registry: Variant, p_inventory: Node, p_spawner: Node) -> void:
...
func get_snapshot() -> Dictionary:
	...
	return {
		...
		"maximum": int(registry.call("get_max_managed_animals")),
		"species_count": int(registry.call("species_count")),
	}


func get_record(husbandry_id: String) -> Dictionary:
	var value: Variant = records.get(husbandry_id, {})
...
func serialize() -> Dictionary:
	...
	for raw_id: Variant in records:
		var husbandry_id: String = str(raw_id)
		var raw_record: Variant = records[raw_id]
		if raw_record is Dictionary:
			serialized_records[husbandry_id] = _serialized_record(raw_record)
	return {
...
func deserialize(data: Dictionary) -> bool:
	...
	var saved_at: int = int(data.get("saved_at_unix", now))
	var offline_cap: float = float(registry.call("get_max_offline_seconds"))
	var elapsed: float = clampf(float(maxi(0, now - saved_at)), 0.0, offline_cap)
	var raw_animals: Variant = data.get("animals", {})
	if raw_animals is not Dictionary:
		return data.is_empty()
	for raw_id: Variant in raw_animals:
		var husbandry_id: String = str(raw_id)
		var raw_record: Variant = raw_animals[raw_id]
		if husbandry_id.is_empty() or raw_record is not Dictionary:
			continue
		var record: Dictionary = _normalize_saved_record(husbandry_id, raw_record, elapsed)
...
func interact_entity(entity: Node, p_inventory: Node = null) -> Dictionary:
	var species_id: String = _species_id(entity)
	if (
		entity == null
		or not is_instance_valid(entity)
		or not entity.is_in_group(GROUP_ANIMALS)
		or not bool(registry.call("supports_species", species_id))
	):
	...
	var selected_value: Variant = active_inventory.call("get_selected_item")
	var selected: Dictionary = selected_value if selected_value is Dictionary else {}
	var selected_item_id: String = str(selected.get("item_id", ""))
	var existing_id: String = _husbandry_id(entity)
	var preview_state: Dictionary = _default_record("", species_id, entity)
	if not existing_id.is_empty() and records.has(existing_id):
		var existing_record: Variant = records.get(existing_id, {})
		if existing_record is Dictionary:
			preview_state = existing_record.duplicate(true)
	var projected_count: int = records.size() + (0 if records.has(existing_id) else 1)
	var profile_value: Variant = registry.call("get_species", species_id)
	var profile: Dictionary = profile_value if profile_value is Dictionary else {}
	var evaluation_value: Variant = policy.call(
		"evaluate_feed",
		profile,
		preview_state,
		selected_item_id,
		projected_count,
		int(registry.call("get_max_managed_animals"))
	)
	var evaluation: Dictionary = evaluation_value if evaluation_value is Dictionary else {}
	if not bool(evaluation.get("success", false)):
		return _reject(
			str(evaluation.get("reason", "feed_rejected")), entity, species_id, evaluation
		)
	var consumed_value: Variant = active_inventory.call("consume_selected", 1)
	var consumed: Dictionary = consumed_value if consumed_value is Dictionary else {}
	...
	var was_managed: bool = not existing_id.is_empty() and records.has(existing_id)
	var previous_record: Dictionary = get_record(existing_id)
	var husbandry_id: String = _ensure_managed(entity, species_id)
	if husbandry_id.is_empty():
		_restore_consumed(active_inventory, consumed)
		return _reject("adoption_failed", entity, species_id, evaluation)
	var action: StringName = StringName(evaluation.get("action", ""))
	match action:
		&"accelerate_growth":
...
func get_entity_prompt(focus: Dictionary, selected_item_id: String) -> Dictionary:
	var species_id: String = str(focus.get("species_id", ""))
	if not bool(registry.call("supports_species", species_id)):
		return {}
	var profile_value: Variant = registry.call("get_species", species_id)
	var profile: Dictionary = profile_value if profile_value is Dictionary else {}
	var entity: Node = _entity_from_focus(focus)
	var husbandry_id: String = _husbandry_id(entity)
	var state: Dictionary = _default_record("", species_id, entity)
	if not husbandry_id.is_empty() and records.has(husbandry_id):
		var raw_state: Variant = records.get(husbandry_id, {})
		if raw_state is Dictionary:
			state = raw_state.duplicate(true)
	var feed_item: String = str(profile.get("feed_item", ""))
	var feed_name: String = _display_name(feed_item)
	var stage: String = str(state.get("stage", PolicyScript.STAGE_ADULT))
	var status: String = "成年 · 可繁殖"
	var secondary: String = "喜欢：%s" % feed_name
	var tone: String = "info"
	if stage == PolicyScript.STAGE_BABY:
		var growth_remaining: float = float(state.get("growth_remaining_seconds", 0.0))
		status = "幼年 · 约 %s 后成年" % _format_duration(growth_remaining)
		if selected_item_id == feed_item:
			secondary = "[鼠标右键] 喂食%s，加速成长" % feed_name
			tone = "success"
	elif float(state.get("breed_cooldown_seconds", 0.0)) > 0.0:
		status = "繁殖冷却 · %s" % _format_duration(
			float(state.get("breed_cooldown_seconds", 0.0))
		)
	elif float(state.get("love_remaining_seconds", 0.0)) > 0.0:
		status = "已准备繁殖 · 还剩 %s" % _format_duration(
...
func _process(delta: float) -> void:
	...
	for raw_id: Variant in records.keys():
		var husbandry_id: String = str(raw_id)
		var raw_record: Variant = records.get(husbandry_id, {})
		if raw_record is not Dictionary:
			continue
		var record: Dictionary = raw_record
		var entity: Node3D = _live_entity(husbandry_id)
		if entity != null:
			record["position"] = _vector3_array(entity.global_position)
			record["health"] = float(
				_property_value(entity, "health", record.get("health", 1.0))
			)
			_apply_simulation_budget(entity)
		var love_before: float = float(record.get("love_remaining_seconds", 0.0))
		var cooldown_before: float = float(record.get("breed_cooldown_seconds", 0.0))
		...
		var grew: bool = false
		if str(record.get("stage", PolicyScript.STAGE_ADULT)) == PolicyScript.STAGE_BABY:
			var growth_before: float = float(record.get("growth_remaining_seconds", 0.0))
			var growth_after: float = maxf(0.0, growth_before - safe_delta)
		...
		if grew:
			...
			var grow_result: Dictionary = _result_for_record(husbandry_id)
			grow_result["message"] = "%s 已经长大" % str(
				grow_result.get("display_name", "动物")
			)
...
func _commit_baby_feed(
	...
	var record: Dictionary = get_record(husbandry_id)
	...
	records[husbandry_id] = record
	_apply_record_to_entity(husbandry_id)
	var result: Dictionary = _result_for_record(husbandry_id)
	result.merge(evaluation, true)
	result["handled"] = true
	result["success"] = true
	result["action"] = &"feed_baby"
	result["message"] = "%s 的成长加速了" % str(
		result.get("display_name", "幼年动物")
	)
...
func _commit_adult_feed(
	...
	records[husbandry_id] = record
	var pair_id: String = _find_pair(husbandry_id)
	if pair_id.is_empty():
		var ready_result: Dictionary = _result_for_record(husbandry_id)
		...
		_emit_state_changed(entity)
		return ready_result
	if records.size() >= int(registry.call("get_max_managed_animals")):
		_rollback_target(husbandry_id, entity, was_managed, previous_record)
		_restore_consumed(active_inventory, consumed)
		return _reject("population_cap", entity, _species_id(entity), evaluation)
	var baby_id: String = _spawn_baby(husbandry_id, pair_id, profile)
	if baby_id.is_empty():
		_rollback_target(husbandry_id, entity, was_managed, previous_record)
		_restore_consumed(active_inventory, consumed)
		return _reject("spawn_failed", entity, _species_id(entity), evaluation)
	var cooldown: float = float(profile.get("breed_cooldown_seconds", 180.0))
	for parent_id: String in [husbandry_id, pair_id]:
		var parent_record: Dictionary = get_record(parent_id)
	...
	var result: Dictionary = _result_for_record(baby_id)
	result["handled"] = true
	result["success"] = true
...
func _find_pair(husbandry_id: String) -> String:
	if not records.has(husbandry_id):
		return ""
	var first: Dictionary = get_record(husbandry_id)
	var first_entity: Node3D = _live_entity(husbandry_id)
	if first_entity == null:
		return ""
	for raw_other_id: Variant in records:
		var other_id: String = str(raw_other_id)
		if other_id == husbandry_id:
			continue
		var other_entity: Node3D = _live_entity(other_id)
		if other_entity == null:
			continue
		var other: Dictionary = get_record(other_id)
		var can_pair: bool = bool(
			policy.call(
				"can_pair",
				first,
				other,
				first_entity.global_position.distance_to(other_entity.global_position),
				float(registry.call("get_pair_radius"))
			)
		)
		if can_pair:
			return other_id
...
func _spawn_baby(first_id: String, second_id: String, profile: Dictionary) -> String:
	if spawner == null or not spawner.has_method("spawn_creature"):
		return ""
	var first_entity: Node3D = _live_entity(first_id)
	var second_entity: Node3D = _live_entity(second_id)
	if first_entity == null or second_entity == null:
		return ""
	var spawn_position: Vector3 = (
		(first_entity.global_position + second_entity.global_position) * 0.5
		+ Vector3(0.35, 0.1, 0.0)
	)
	...
	var husbandry_id: String = _next_id()
	var record: Dictionary = _default_record(
		husbandry_id, str(profile.get("id", "")), child
	)
...
func _ensure_managed(entity: Node, species_id: String) -> String:
	...
	var record: Dictionary = _default_record(husbandry_id, species_id, entity)
	records[husbandry_id] = record
	_register_live_creature(husbandry_id, entity)
	return husbandry_id
...
func _register_live_creature(husbandry_id: String, entity: Node) -> void:
	...
	if not entity.has_meta(META_BASE_NAME):
		entity.set_meta(
			META_BASE_NAME, str(_property_value(entity, "display_name", entity.name))
		)
	if not entity.is_in_group(GROUP_PERSISTENT):
		entity.add_to_group(GROUP_PERSISTENT)
	if entity.has_signal("died"):
		var callback: Callable = Callable(self, "_on_creature_died").bind(husbandry_id)
...
func _restore_managed_animals() -> void:
	...
	for raw_id: Variant in records.keys():
		...
		if _live_entity(husbandry_id) != null:
			continue
		var record: Dictionary = get_record(husbandry_id)
		var position: Vector3 = _vector3_from_value(record.get("position", []))
...
func _apply_record_to_entity(husbandry_id: String) -> void:
	...
	var profile_value: Variant = registry.call(
		"get_species", str(record.get("species_id", ""))
	)
	var profile: Dictionary = profile_value if profile_value is Dictionary else {}
	var baby: bool = (
		str(record.get("stage", PolicyScript.STAGE_ADULT)) == PolicyScript.STAGE_BABY
	)
	var scale_value: float = float(registry.call("get_baby_scale")) if baby else 1.0
	entity.scale = Vector3.ONE * scale_value
	var base_name: String = str(
		profile.get("name", entity.get_meta(META_BASE_NAME, entity.name))
	)
	_set_property_if_present(entity, "display_name", "幼年%s" % base_name if baby else base_name)
	var maximum: float = maxf(
		1.0, float(_property_value(entity, "max_health", 1.0))
	)
...
func _apply_simulation_budget(entity: Node3D) -> void:
	if player == null or not is_instance_valid(player):
		return
	var radius: float = float(registry.call("get_simulation_radius"))
	var should_simulate: bool = (
		entity.global_position.distance_squared_to(player.global_position) <= radius * radius
	)
...
func _default_record(husbandry_id: String, species_id: String, entity: Node) -> Dictionary:
	var position: Vector3 = entity.global_position if entity is Node3D else Vector3.ZERO
	var health_value: float = float(_property_value(entity, "health", 1.0))
	return {
		"id": husbandry_id,
...
func _normalize_saved_record(
	...
	var species_id: String = str(raw_record.get("species_id", ""))
	if not bool(registry.call("supports_species", species_id)):
		return {}
	var position: Vector3 = _vector3_from_value(raw_record.get("position", []))
	if not _vector3_is_finite(position):
		return {}
	var stage: String = str(raw_record.get("stage", PolicyScript.STAGE_ADULT))
	if stage not in [PolicyScript.STAGE_ADULT, PolicyScript.STAGE_BABY]:
		stage = PolicyScript.STAGE_ADULT
	var growth_remaining: float = maxf(
	...
	return {
		...
		"growth_remaining_seconds": (
			growth_remaining if stage == PolicyScript.STAGE_BABY else 0.0
		),
...
func _serialized_record(record: Dictionary) -> Dictionary:
	...
	var serialized_position: Array = (
		position_value.duplicate(true) if position_value is Array else []
	)
	return {
...
func _result_for_record(husbandry_id: String) -> Dictionary:
	...
	var profile: Dictionary = profile_value if profile_value is Dictionary else {}
	var baby: bool = str(record.get("stage", "")) == PolicyScript.STAGE_BABY
	return {
...
func _reject(reason: String, entity: Node, species_id: String, context: Dictionary) -> Dictionary:
	...
	var profile_value: Variant = registry.call("get_species", species_id)
	var profile: Dictionary = profile_value if profile_value is Dictionary else {}
	result["display_name"] = str(profile.get("name", species_id))
...
func _message_for_reason(reason: String, species_id: String, context: Dictionary) -> String:
	var profile_value: Variant = registry.call("get_species", species_id)
	var profile: Dictionary = profile_value if profile_value is Dictionary else {}
	var animal_name: String = str(profile.get("name", "动物"))
	var feed_name: String = _display_name(str(context.get("feed_item", "")))
	match reason:
		"wrong_feed":
		...
		"breed_cooldown":
			return "%s还需 %s 才能再次繁殖" % [
				animal_name,
				_format_duration(float(context.get("remaining_seconds", 0.0))),
			]
		"already_ready":
			return "%s已经准备繁殖，请寻找附近同类" % animal_name
		"population_cap":
			return "已达到可管理动物上限（%d）" % int(
				registry.call("get_max_managed_animals")
			)
		"spawn_failed":
			return "幼崽出生位置不可用，饲料已退回"
		"consume_failed", "target_changed":
			return "饲料或动物状态发生变化，本次喂食已取消"
...
func _restore_consumed(active_inventory: Node, consumed: Dictionary) -> void:
	if consumed.is_empty() or active_inventory == null or not active_inventory.has_method("add_item"):
		return
	var item_id: String = str(consumed.get("item_id", ""))
	var count: int = maxi(0, int(consumed.get("count", 0)))
	if item_id.is_empty() or count <= 0:
		return
	var metadata_value: Variant = consumed.get("metadata", {})
	var metadata: Dictionary = (
		metadata_value.duplicate(true) if metadata_value is Dictionary else {}
	)
	var remaining: int = int(active_inventory.call("add_item", item_id, count, metadata))
	if remaining > 0:
		push_error("Husbandry rollback could not restore %d x %s" % [remaining, item_id])


func _next_id() -> String:
	var base: int = int(Time.get_unix_time_from_system())
	while true:
		_id_counter += 1
		var candidate: String = "animal@%d-%d" % [base, _id_counter]
...
func _entity_from_focus(focus: Dictionary) -> Node:
	var entity_id: int = int(focus.get("entity_id", 0))
	if entity_id <= 0:
		return null
...
func _format_duration(seconds: float) -> String:
	return str(policy.call("format_duration", seconds))
