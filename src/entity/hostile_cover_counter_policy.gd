...
const NON_BLOCKING_IDS: Array[String] = [
	"air", "water", "lava", "leaves", "ladder", "ladder_east", "ladder_north",
	"ladder_west", "torch", "tall_grass", "flower_red", "flower_yellow",
	"dead_bush", "glow_crystal", "wheat_stage_0", "wheat_stage_1",
	"wheat_stage_2", "wheat_stage_3", "carrot_stage_0", "carrot_stage_1",
	"carrot_stage_2", "carrot_stage_3", "potato_stage_0", "potato_stage_1",
	"potato_stage_2", "potato_stage_3",
]
...
static func line_samples(
	...
	var step_count := clampi(ceili(distance * 2.0), 1, mini(MAX_LINE_SAMPLE_STEPS, maxi(1, maximum_steps)))
...
static func blocks_projectile_lane(block_id: String, local_height: float = 0.5) -> bool:
	if block_id in NON_BLOCKING_IDS:
		return false
	if block_id.begins_with("oak_door_open") or block_id.begins_with("oak_door_upper_open"):
		return false
