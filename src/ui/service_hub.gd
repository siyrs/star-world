...
const DEFAULT_SETTINGS := {"mouse_sensitivity":0.18, "render_distance":4, "master_volume":0.8, "fullscreen":false, "cycle_minutes":10}
...
func _ready() -> void:
	...
	current_settings["render_distance"] = clampi(int(current_settings.get("render_distance", DEFAULT_SETTINGS.render_distance)), 1, 5)
...
func _begin_world(state: Dictionary) -> void:
	...
	audio_service.start_ambient(str(profile.get("ambient", _ambient_for_map(str(metadata.get("map_id", ""))))))
	if main_menu != null: main_menu.visible = false
	if game_ui != null: game_ui.visible = true
	start_world_requested.emit(current_state.duplicate(true))


func attach_game(world, player: Node3D, sun: DirectionalLight3D = null, environment: WorldEnvironment = null, ground_resolver: Callable = Callable()) -> void:
	world_node = world
	player_node = player
	...
	if player != null:
		...
		if player.has_method("setup_gameplay_services"):
			player.call("setup_gameplay_services", {"inventory":inventory, "survival":survival, "audio":audio_service, "game_ui":game_ui})
...
func _on_settings_changed(settings: Dictionary) -> void:
	...
	merged["render_distance"] = clampi(int(merged.get("render_distance", DEFAULT_SETTINGS.render_distance)), 1, 5)
...
func _apply_settings(settings: Dictionary) -> void:
	var volume := clampf(float(settings.get("master_volume", DEFAULT_SETTINGS.master_volume)), 0.0, 1.0)
	...
	day_night.cycle_duration_seconds = maxf(1.0, float(settings.get("cycle_minutes", DEFAULT_SETTINGS.cycle_minutes))) * 60.0
	if player_node != null:
		_set_property_if_present(player_node, "mouse_sensitivity", float(settings.get("mouse_sensitivity", DEFAULT_SETTINGS.mouse_sensitivity)) / 100.0)
	if world_node != null:
		var render_distance := clampi(int(settings.get("render_distance", DEFAULT_SETTINGS.render_distance)), 1, 5)
...
func return_to_menu() -> void:
	...
	if game_ui != null: game_ui.visible = false
	if main_menu != null: main_menu.show_main()
...
func _player_state(player: Node3D) -> Dictionary:
	return {"position":[player.global_position.x, player.global_position.y, player.global_position.z], "rotation":[player.rotation.x, player.rotation.y, player.rotation.z]}


func _ambient_for_map(map_id: String) -> String:
	return {"star_continent":"forest", "desert_ruins":"desert", "frozen_wastes":"wind", "sky_islands":"sky", "abyss_world":"cave"}.get(map_id, "forest")
