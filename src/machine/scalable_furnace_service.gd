class_name ScalableFurnaceService
extends "res://src/machine/furnace_service.gd"
...
func _is_runnable_state(state: Dictionary) -> bool:
	...
	if recipe.is_empty() or not _can_accept_output(state, recipe):
		return false
	if float(state.get("burn_remaining_seconds", 0.0)) > EPSILON:
		return true
	var fuel_slot: Dictionary = state.get(SLOT_FUEL, {})
	return (
		int(fuel_slot.get("count", 0)) > 0
		and fuels.is_fuel(str(fuel_slot.get("item_id", "")))
	)
