...
const AdapterScript = preload("res://src/agriculture/agriculture_interaction_adapter.gd")
const BlockRegistryScript = preload("res://src/block/block_registry.gd")
const CaptureConfig = preload("res://tests/qa/desktop_capture_config.gd")

const OUTPUT_PATH := "user://fertilizer-desktop-acceptance.png"
...
class DesktopFertilizerWorld:
	...
	func build() -> void:
		_build_floor()
		set_test_block(SOIL_POSITION, "farmland_wet")
		set_test_block(CROP_POSITION, "wheat_stage_0")
	...
	func _refresh_block_node(position: Vector3i, block_id: String) -> void:
		...
		body.name = "FertilizerBlock_%s" % key.replace(",", "_")
...
func _run() -> void:
	...
	var host := Node3D.new()
	root.add_child(host)
	var world = DesktopFertilizerWorld.new()
	...
	for node in [world, inventory, tools, agriculture, adapter, interactions, player]:
		host.add_child(node)
	world.build()
	await process_frame
	await process_frame
	...
	agriculture.deserialize(
		{
			...
			"soil_moisture": {
				...
				"soils": {
					"soil@0,1,-3": {
						"position": [0, 1, -3],
						"block_id": "farmland_wet",
						"manual_remaining_seconds": 120.0,
					}
				},
			},
		}
	...
	adapter.setup(agriculture)
	interactions.setup(null, null, inventory, null)
	interactions.register_extension(adapter)
	inventory.clear()
	inventory.add_item("compost", 2)
	...
	await physics_frame
	await physics_frame
	_check(root.get_camera_3d() == player.get_view_camera(), "the player camera owns the fertilizer desktop viewport")
	_check(
		str(interactions.get_interaction_hint_for_item("wheat_stage_0", "compost")).contains("施用堆肥"),
		"compost exposes a clear contextual action",
	)
	await _aim_at(player, Vector3(SOIL_POSITION) + Vector3(0.5, 0.5, 0.5))
	_check(_ray_hits_block(player, world, SOIL_POSITION), "the real ray resolves supporting farmland")
	player.call("_update_interaction_focus", true)
	var focus: Dictionary = player.get_interaction_focus()
	_check(str(focus.get("block_id", "")) == "wheat_stage_0", "focus proxy exposes the non-colliding wheat crop")
	await _right_click()
	_check(world.get_block(CROP_POSITION) == "wheat_stage_1", "a real right click advances wheat by one stage")
	_check(inventory.count_item("compost") == 1, "desktop fertilization consumes one compost")
	_check(int(agriculture.get_crop_state(CROP_POSITION).get("stage", -1)) == 1, "desktop fertilization updates domain state")
	player.call("_update_interaction_focus", true)
	var next_focus: Dictionary = player.get_interaction_focus()
	_check(str(next_focus.get("block_id", "")) == "wheat_stage_1", "focus refreshes to the fertilized stage")
	await RenderingServer.frame_post_draw
	var image := root.get_texture().get_image()
	_check(image != null and not image.is_empty(), "fertilizer desktop viewport produces a rendered frame")
	if image != null and not image.is_empty():
		_save_image(image)
	_check(Input.mouse_mode == Input.MOUSE_MODE_CAPTURED, "fertilizer interaction preserves gameplay mouse capture")
	...
	if failures.is_empty():
		print(
			"QA FERTILIZER DESKTOP PASS | checks=%d | capture=%s"
			% [checks, _capture_path]
		)
		quit(0)
	else:
		for failure: String in failures:
			push_error("QA FERTILIZER DESKTOP FAILURE: %s" % failure)
		print(
			"QA FERTILIZER DESKTOP FAIL | checks=%d | failures=%d"
...
func _save_image(image: Image) -> void:
	...
	_check(
		...
		"fertilizer desktop screenshot is saved",
	)
