...
func _test_registry_and_policy() -> void:
	...
	_check(registry.schema_version == 2, "prospecting schema version is stable")
	_check(registry.get_validation_errors().is_empty(), "prospecting data has no structural errors")
	_check(registry.validate_item_registry(item_registry), "all prospecting item capabilities are registered")
	_check(registry.get_tool_item_id() == "prospecting_kit", "registry preserves the canonical base prospecting tool")
	_check(registry.get_tool_ids().size() == 6, "registry exposes the base tool and five map calibrations")
	var config := registry.get_config()
	_check(int(config.get("max_samples", 0)) <= 768, "base scan sample budget stays under the calibrated hard limit")
	_check(int(config.get("max_records", 0)) == 64, "persistent discovery history is bounded")
	for tool_id: String in registry.get_tool_ids():
		var tool_config := registry.get_tool_config(tool_id)
		_check(int(tool_config.get("theoretical_samples", 0)) <= int(tool_config.get("max_samples", 0)), "%s theoretical samples fit the hard budget" % tool_id)
		_check(int(tool_config.get("max_samples", 0)) <= 768, "%s respects the global calibrated sample limit" % tool_id)
...
func _test_service_lifecycle() -> void:
	...
	var wrong_calibration: Dictionary = service.use_item("verdant_prospecting_kit", 1000)
	_check(str(wrong_calibration.get("reason", "")) == "wrong_calibration", "map-specific calibration rejects the wrong world")
	var result: Dictionary = service.use_item("prospecting_kit", 1000)
	_check(bool(result.get("handled", false)) and bool(result.get("success", false)), "base prospecting kit completes a bounded scan")
	_check(int(result.get("sample_count", 0)) <= 637, "base runtime scan respects its original sample budget")
	_check(int(result.get("geology_samples", 0)) >= 24, "runtime scan has enough geology evidence")
	_check(str(result.get("calibration_id", "")) == "basic", "base scan reports its calibration identity")
	...
	_check((first_snapshot.get("tool_item_ids", []) as Array).size() == 6, "service snapshot exposes all accepted prospecting tools")
	...
	_check(bool(second.get("success", false)), "base scan works again after cooldown")
	_check(int(service.get_snapshot().get("record_count", 0)) == 2, "a new chunk creates a second persistent discovery")
	player.global_position = Vector3(50.5, 20.0, -18.5)
	var calibrated: Dictionary = service.use_item("abyss_prospecting_kit", 3200)
	_check(bool(calibrated.get("success", false)), "matching abyss calibration performs a production scan")
	_check(str(calibrated.get("calibration_id", "")) == "abyss", "calibrated scan reports its map identity")
	_check(int(calibrated.get("scan_profile", {}).get("vertical_radius", 0)) == 18, "abyss calibration applies the deeper vertical radius")
	_check(int(calibrated.get("sample_count", 0)) <= 684, "abyss calibration remains within its hard sample budget")
	_check(int(service.get_snapshot().get("record_count", 0)) == 3, "calibrated scan persists through the same discovery contract")
	...
	_check((serialized.get("records", []) as Array).size() == 3, "exploration records serialize")
	var restored = ProspectingServiceScript.new()
	root.add_child(restored)
	restored.setup(item_registry)
	restored.deserialize(serialized)
	_check(int(restored.get_snapshot().get("record_count", 0)) == 3, "exploration records deserialize")
	_check(not restored.get_record(str(calibrated.get("record_key", ""))).is_empty(), "calibrated record lookup survives reload")
	world.geology_enabled = false
	restored.attach_world(world, player)
	var rejected: Dictionary = restored.use_item("abyss_prospecting_kit", 4000)
	_check(str(rejected.get("reason", "")) == "insufficient_geology", "air-only calibrated scan is rejected as insufficient geology")
...
func _test_player_experience_contracts() -> void:
	...
	inventory.selected_item_id = "abyss_prospecting_kit"
	root.add_child(inventory)
	var prompt_resolver = PromptResolverScript.new()
	var prompt: Dictionary = prompt_resolver.resolve({}, inventory, null)
	_check(bool(prompt.get("visible", false)), "holding a calibrated prospecting kit exposes an interaction prompt")
