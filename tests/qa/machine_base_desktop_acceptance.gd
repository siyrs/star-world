...
func _run() -> void:
	...
	_check(scheduler.call("is_active"), "world activation starts the shared machine scheduler")
	_check(furnace.is_externally_scheduled() and not furnace.is_processing(), "production furnace has no duplicate private process loop")
	...
	_check(furnace.ensure_machine(first_id) and furnace.ensure_machine(second_id), "two production furnace instances register with stable ids")
	hub.inventory.clear()
	...
	_check(furnace.transfer_from_inventory(hub.inventory, 0, furnace.SLOT_INPUT, first_id), "first furnace receives iron input")
	_check(furnace.transfer_from_inventory(hub.inventory, 2, furnace.SLOT_FUEL, first_id), "first furnace receives its own fuel stack")
	_check(furnace.transfer_from_inventory(hub.inventory, 1, furnace.SLOT_INPUT, second_id), "second furnace receives gold input")
	_check(furnace.transfer_from_inventory(hub.inventory, 3, furnace.SLOT_FUEL, second_id), "second furnace receives its own fuel stack")
	var first_before: Dictionary = furnace.get_machine_snapshot(first_id)
	...
	_check(panel != null and panel.get_active_machine_id() == first_id, "real furnace panel uses the stable machine id")
	...
	var first_after: Dictionary = furnace.get_machine_snapshot(first_id)
	var second_after: Dictionary = furnace.get_machine_snapshot(second_id)
	...
	for _frame in 8:
		await process_frame
	_check(not scheduler.call("is_active"), "return-to-menu stops shared machine processing")
	_check(int(furnace.get_runtime_snapshot().get("machine_count", -1)) == 0, "return-to-menu clears machine runtime state")
	...
	_check(scheduler.call("is_active"), "full reload reactivates the same shared scheduler")
	_check(int(furnace.get_machine_snapshot(first_id).get("output", {}).get("count", 0)) == 1, "full reload restores iron output exactly once")
	_check(int(furnace.get_machine_snapshot(second_id).get("output", {}).get("count", 0)) == 1, "full reload restores gold output exactly once")
