...
func _test_registry_extension_and_crafting() -> void:
	...
	_check(inventory.registry.has_item("flint"), "default item registry atomically includes flint crafting input")
	...
	_check(int(crafting.get_recipe("arrows").get("ingredients", {}).get("flint", 0)) == 1, "arrow recipe consumes one flint rather than raw stone")
	inventory.clear()
