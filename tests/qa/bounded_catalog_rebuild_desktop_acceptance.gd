...
const OUTPUT_PATH := "user://bounded-catalog-rebuild-desktop.png"
const WORLD_COUNT := 24
const CATALOG_REBUILD_BUDGET := 16
const OVERRIDES_PER_WORLD := 64
const CLEANUP_FRAMES := 20
...
func _run() -> void:
	...
	_check(
		...
		"first desktop refresh uses exactly sixteen catalog write slots"
	)
	_check(
		int(first.get("last_deferred_catalog_rebuild_count", -1)) == 8,
		"first desktop refresh defers the remaining eight sidecars"
	)
	_check(
		int(first.get("last_repair_budget_used", -1)) == 0,
		"catalog-only recovery does not consume primary repair slots"
	)
	_check(
		status_label.text.contains("待建目录 8")
		and status_label.text.contains("每次最多 16"),
	...
	_check(
		int(projected_catalog.get("last_deferred_catalog_rebuild_count", -1)) == 8
		and int(projected_catalog.get("catalog_rebuild_budget", -1))
		== CATALOG_REBUILD_BUDGET,
		"runtime health keeps the bounded catalog backlog"
	)
	_check(
		str(operations.get("primary_bottleneck", {}).get("id", "")) == "catalog",
		"deferred sidecar rebuilding becomes the deterministic health bottleneck"
	)
	...
	_check(
		...
		"real F3 input opens the bounded catalog health view"
	)
	var display := str(overlay.call("get_display_text")) if overlay != null else ""
	_check(
		display.contains("待建目录 8") and display.contains("目录写入预算 16"),
	...
	_check(
		...
		and int(second.get("last_repair_count", -1)) == 8
		and int(second.get("last_deferred_catalog_rebuild_count", -1)) == 0,
	...
	_check(
		...
		and int(steady.get("last_repair_count", -1)) == 0,
		"steady desktop refresh is a pure sidecar hit"
	)
	_check(
		int(steady.get("last_catalog_rebuild_budget_used", -1)) == 0,
		"steady desktop refresh performs zero catalog writes"
	)
	_check(
		_visible_fixture_rows(list_node) == WORLD_COUNT,
		"all world rows remain visible after sidecar convergence"
	)
	var recovery: Dictionary = save.get_recovery_diagnostics()
	_check(
		int(recovery.get("recovery_count", 0)) == 0,
		"desktop sidecar convergence never enters backup recovery"
	)
	for world_id: String in world_ids:
		_check(
			_read_text(_world_path(world_id))
			== str(primary_text_by_world.get(world_id, "")),
			"desktop sidecar convergence preserves primary %s" % world_id
		)
...
func _create_fixture(save: Node) -> void:
	var prefix := "Desktop-Catalog-Rebuild-%d" % Time.get_ticks_msec()
	for index in WORLD_COUNT:
		...
		for offset in OVERRIDES_PER_WORLD:
			overrides["%d,16,%d" % [index * 96 + offset, index]] = "stone_bricks"
...
func _visible_fixture_rows(list_node: VBoxContainer) -> int:
	...
	for row: Node in list_node.get_children():
		if row.get_child_count() > 0:
			...
			if button != null and button.text.contains("Desktop-Catalog-Rebuild"):
				count += 1
	return count
...
func _write_report() -> void:
	...
	if file == null:
		_check(false, "desktop catalog JSON report opens")
		return
	file.store_string(JSON.stringify(report, "\t"))
	file.close()
	_check(FileAccess.file_exists(report_path), "desktop catalog JSON report is saved")
...
func _finish(game: Node, save: Node) -> void:
	...
	if failures.is_empty():
		print(
			"QA BOUNDED CATALOG REBUILD DESKTOP PASS | checks=%d | worlds=%d"
			% [checks, WORLD_COUNT]
		)
		quit(0)
	else:
		for failure: String in failures:
			push_error("QA BOUNDED CATALOG REBUILD DESKTOP FAILURE: %s" % failure)
		print(
			"QA BOUNDED CATALOG REBUILD DESKTOP FAIL | checks=%d | failures=%d"
