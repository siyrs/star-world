...
func _run() -> void:
	...
	_check(hub != null and runtime != null and coordinator != null, "production game mounts the composed exploration runtime")
	if hub == null or runtime == null or coordinator == null:
	...
	_check(int(coordinator.call("get_snapshot").get("participant_count", 0)) == 4, "production coordinator retains all four lifecycle participants")
	...
	for index in HOSTILE_COUNT:
		...
		var position := player.global_position + Vector3(
			sin(angle) * 8.0, 0.0, cos(angle) * 8.0
		)
	...
	_check(
		int(lifecycle_after_spawn.get("immediate_refresh_count", 0))
		== refresh_before_spawn + 1,
		"five synchronous ecology events perform one danger assessment"
	)
	_check(
		int(lifecycle_after_spawn.get("immediate_event_count", 0))
		>= events_before_spawn + HOSTILE_COUNT,
		"runtime diagnostics retain all raw spawn events"
	)
	...
	var events_before_windup := int(
		lifecycle_after_spawn.get("immediate_event_count", 0)
	)
	...
	for index in hostiles.size():
		...
		hostile.global_position = player.global_position + Vector3(
			sin(angle) * 1.25, 0.0, cos(angle) * 1.25
		)
		hostile.set("target", player)
	...
	_check(
		int(lifecycle_after_windup.get("immediate_refresh_count", 0))
		== refresh_before_windup + 1,
		"five simultaneous attack-state events perform one danger assessment"
	)
	_check(
		int(lifecycle_after_windup.get("immediate_event_count", 0))
		>= events_before_windup + HOSTILE_COUNT,
		"runtime diagnostics preserve every windup event"
	)
	_check(
		int(lifecycle_after_windup.get("coalesced_danger_event_count", 0)) >= 8,
		"runtime diagnostics prove that spawn and windup work was coalesced"
	)
	...
	var refresh_before_cancel := int(
		lifecycle_after_windup.get("immediate_refresh_count", 0)
	)
	for hostile: Node3D in hostiles:
	...
	_check(
		int(lifecycle_after_cancel.get("immediate_refresh_count", 0))
		== refresh_before_cancel + 1,
		"five synchronous cancellations perform one danger assessment"
	)
	...
	_check(
		"phase_changed" in last_triggers
		and "ecology_changed" in last_triggers
		and "threat_changed" in last_triggers,
	...
	_check(
		not loaded.has("danger_refresh")
		and not loaded.has("hostile_windups")
...
func _save_image(image: Image) -> void:
	...
	_check(
		...
		"multi-hostile danger screenshot is saved"
	)
...
func _finish(game: Node, hub: Node) -> void:
	...
	if failures.is_empty():
		print(
			"QA MULTI HOSTILE DANGER DESKTOP PASS | checks=%d | capture=%s"
		...
		print(
			"QA MULTI HOSTILE DANGER DESKTOP FAIL | checks=%d | failures=%d"
