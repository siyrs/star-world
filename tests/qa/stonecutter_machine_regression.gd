...
func _test_production_service_hub() -> void:
	...
	_check(int(scheduler_snapshot.get("domain_count", 0)) == 3, "production scheduler owns furnace, stonecutter and automation domains")
	...
	_check(int(character.get("machine_runtime", {}).get("domain_count", 0)) == 3, "character diagnostics expose furnace, stonecutter and automation domains")
