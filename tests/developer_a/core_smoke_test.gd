...
func _run() -> void:
	...
	if failures.is_empty():
		print(
			(
				"DEV-C CORE SMOKE PASS | checks=%d | blocks=%d | profiles=5"
				% [checks, BlockRegistryScript.BLOCK_IDS.size()]
			)
		)
		quit(0)
	else:
...
func _test_block_registry() -> void:
	...
	for required_id in [
		...
		_check(
			BlockRegistryScript.has_block(required_id), "missing required block %s" % required_id
		)
...
func _test_seeded_profiles() -> void:
	var profiles := [
		"star_continent", "desert_ruins", "frozen_wastes", "sky_islands", "abyss_world"
	]
	...
	for profile_id in profiles:
		...
		for sample in [Vector2i(-19, 7), Vector2i(0, 0), Vector2i(13, 29), Vector2i(41, -11)]:
			...
			_check(
				first_height == second_height, "%s height generation is deterministic" % profile_id
			)
			(
				signature
					. append(
						[
							first_height,
							first.get_block(Vector3i(sample.x, maxi(1, first_height), sample.y)),
						]
					)
			)
		...
		_check(
			first.get_block(camera_cell) == BlockRegistryScript.AIR,
			"%s keeps the first-person camera outside solid or leaf blocks" % profile_id,
		)
...
func _test_all_profile_world_boots() -> void:
	...
	for profile_id in [
		...
		_check(
			int(chunk.get("surface_face_count")) > 0,
			"%s produces renderable terrain" % profile_id,
		)
...
func _test_world_api_and_chunks():
	...
	_check(
		int(center_chunk.get("surface_face_count")) > 0, "chunk surface mesh contains visible faces"
	)
	...
	_check(
		saved_world.get("block_overrides", {}).has(world.block_key(target)),
		"placed voxel is serialized sparsely",
	)
	...
	_check(
		restored.get_block(target) == "planks", "sparse override restores in a new world instance"
	)
	...
	for _frame in 64:
		...
		if world.get_loaded_chunk_count() >= 2:
			break
	_check(world.get_loaded_chunk_count() >= 2, "streaming incrementally loads neighboring chunks")
...
func _test_player_contract(world) -> void:
	...
	_check(
		player.get_selected_block_id() == "planks", "1-9 hotbar selection maps to placeable blocks"
	)
	_check(player.get_view_camera() is Camera3D, "first-person camera exists")
	_check(
		player.has_method("break_target_block") and player.has_method("place_selected_block"),
		"mining and placement APIs exist",
	)
	player.queue_free()
...
func _test_game_and_service_integration() -> void:
	...
	_check(
		(
			hub.get("inventory") is Node
			and hub.get("crafting") is Node
			and hub.get("save_service") is Node
		),
		"real inventory/crafting/save services are mounted",
	)
	_check(
		(
			hub.get("survival") is Node
			and hub.get("day_night") is Node
			and hub.get("audio_service") is Node
		),
		"real survival/day-night/audio services are mounted",
	)
	_check(hub.get("creature_spawner") is Node, "creature spawner service is mounted")
	var state := {
		"metadata":
		{
			"id": "developer-a-smoke",
			"name": "Developer A Smoke",
			"map_id": "desert_ruins",
			"seed": 998877,
		},
		"inventory": {},
	...
	_check(
		...
		"hub start signal launches voxel world"
	)
	_check(
		str(game.world.get("profile_id")) == "desert_ruins",
		"map metadata selects the requested profile"
	)
	_check(
		game.player.get("inventory") == hub.get("inventory"),
		"player is bound to the real inventory service"
	)
	_check(
		game.world_root.visible and game.player.visible,
		"gameplay becomes visible after world start"
	)
	var collected: Dictionary = game.request_save()
	_check(
		collected.get("world", {}).has("block_overrides"),
		"save payload includes sparse world overrides"
	)
	var save_service = hub.get("save_service")
	_check(
		save_service.world_exists("developer-a-smoke"), "hub save_current persists the active world"
	)
...
func _test_project_delivery_files() -> void:
	...
	_check(
		'run/main_scene="res://scenes/game/game.tscn"' in project_text,
		"project boots through integrated game/menu scene",
	)
	_check(FileAccess.file_exists("res://export_presets.cfg"), "Windows export preset exists")
	var export_text := FileAccess.get_file_as_string("res://export_presets.cfg")
	_check(
		'platform="Windows Desktop"' in export_text, "Windows Desktop export target is configured"
	)
