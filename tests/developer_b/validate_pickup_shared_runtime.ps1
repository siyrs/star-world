...
foreach ($constant in @(
  'MAX_RUNTIME_NODES\s*:=\s*128',
  'MAX_RUNTIME_DELTA_SECONDS\s*:=\s*0\.25'
)) {
...
foreach ($phrase in @(
  ...
  if ($text.Regression -notmatch [regex]::Escape($phrase)) {
    throw "Shared pickup runtime regression is missing assertion: $phrase"
  }
}
foreach ($phrase in @(
  ...
  if ($text.Desktop -notmatch [regex]::Escape($phrase)) {
    throw "Shared pickup runtime desktop acceptance is missing assertion: $phrase"
  }
}
if ($text.Workflow -notmatch 'pickup_shared_runtime_regression\.gd' -or $text.Workflow -notmatch 'pickup_shared_runtime_desktop_acceptance\.gd') {
  throw 'Shared pickup runtime workflow must run domain and real desktop acceptance'
}
if ($text.Workflow -notmatch 'pickup-shared-runtime-desktop\.json' -or $text.Workflow -notmatch 'Invoke-Godot\.ps1') {
  throw 'Shared pickup runtime workflow must upload awaited machine-readable evidence'
}
...
Write-Host 'PASS pickup_shared_runtime processes=1 nodes=128 delta=0.25 resources=shared materials=256 anchors=stable pause=pausable persistence=none'
