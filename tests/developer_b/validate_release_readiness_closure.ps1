...
Require-Contains $smoke '"version": 5' 'Release smoke report schema must advertise route, performance and authoritative lifecycle evidence'
Require-Contains $smoke 'request_application_quit' 'Release smoke must finalize through the production quit coordinator'
Require-Contains $smoke 'prepared_quit' 'Release smoke must require a prepared authoritative quit lifecycle'
...
Require-Contains $smokeDriver 'Get-AuthoritativeLifecycleEvidence' 'Release driver must parse clean lifecycle semantics instead of trusting a hash'
