...
Assert-ContainsAll $pinWriter @('star-world-release-promotion-pin-v1', 'release_owner_id', 'bundle_id', 'validate_release_promotion_pin.ps1')
Assert-ContainsAll $pinValidator @('Expected promotion pin mismatch', 'pin_id does not match the promotion identity', 'candidate_id', 'bundle_id')
...
Assert-ContainsAll $bundleValidator @('-RequireReleaseGate requires an externally retained -ExpectedPinId', 'offline_contract_validation', 'Unsafe promotion path', 'Promotion bundle contains missing or unexpected physical files', 'star-world-promotion-contracts-')
...
Assert-ContainsAll $bundleTest @('Contract snapshot tamper', 'Wrong expected promotion pin', 'Hidden promotion file injection', 'Promotion identity tamper', 'negative_cases=8')
...
& $bundleTest
Write-Host 'ITERATION 62 RELEASE PROMOTION PASS | offline-contracts=true | external-pin=true | immutable-receipt=true | negative-cases=8'
