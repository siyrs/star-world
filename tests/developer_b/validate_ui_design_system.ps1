...
Assert-Matches $text.main_menu 'get_visible_rect|PANEL_SAFE_MARGIN|_apply_responsive_layout' 'Main menu must own responsive viewport safety'
Assert-Matches $text.protected_menu 'protected_save_browser_panel\.gd' 'Production menu must preserve protected save management'
...
Write-Host 'PASS ui_design_system palette=celestial hierarchy=semantic spacing=8pt focus=visible menus=responsive overlays=unified screens=10 desktop=real-input release=required'
