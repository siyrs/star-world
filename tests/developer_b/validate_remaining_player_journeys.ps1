...
$explorationRewardPersistence = Read-RepoFile 'tests/qa/exploration_reward_inventory_persistence_regression.gd'
...
Require-Contains $agricultureDesktop 'MAX_TEST_SECONDS := 600.0' 'Agriculture journey must fail within a bounded watchdog window'
Require-Contains $agricultureDesktop '_last_rejection_reason(rejections)' 'Agriculture rejection assertions must not abort on a missing event'
Require-Contains $agricultureDesktop 'player.move_and_slide()' 'Agriculture collision evidence must use a live CharacterBody3D collision query'
Require-Contains $agricultureDesktop 'WATCHDOG: stage=%s' 'Agriculture timeout evidence must retain the last completed stage'
...
Require-Contains $explorationDesktop '_canonical_inventory_payload(loaded.get("inventory", {})) == claimed_inventory' 'Exploration world JSON must cross the canonical inventory boundary before exact comparison'
Require-Contains $explorationDesktop 'InventoryScript.new(36, 9)' 'Exploration canonical comparison must use the production inventory service'
Require-Contains $explorationRewardPersistence 'SaveService JSON plus InventoryService canonicalization preserves exact slots and metadata' 'Independent exploration reward persistence must verify the same canonical boundary'
...
Require-Contains $workflow 'exploration_closed_loop_desktop_acceptance.gd' 'Permanent CI must run the exploration closed loop directly'
Require-Contains $workflow 'exploration_reward_inventory_persistence_regression.gd' 'Permanent CI must independently run exploration reward persistence'
Require-NotContains $workflow 'exploration_closed_loop_canonical_desktop_acceptance.gd' 'Permanent CI may not bypass exploration assertions through a wrapper'
...
Write-Host '  - agriculture failure, growth, harvest and exact reload are bounded and covered'
Write-Host '  - bed spawn, real respawn, removal fallback and exact reload are covered'
Write-Host '  - complete exploration milestones, canonical reward persistence and reload are covered'
